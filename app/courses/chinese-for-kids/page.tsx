...
export async function generateMetadata(): Promise<Metadata> {
  return buildMetadata({
    title: "Китайский для детей от 7 лет онлайн — ChinaChild",
    description:
      "Курс китайского для детей от 7 лет и подростков: программа по возрасту, индивидуальный модуль от 17 990 ₽ за месяц и мини-группы. Налоговый вычет для родителя.",
...
const whyCards = [
  {
    ...
    title: "С 7 лет, без зубрёжки",
    body:
      "Фонетика, тоны, базовые иероглифы и живые диалоги. Для младших школьников добавляем игровые задания, для подростков — системную подготовку к HSK 1–2.",
  },
  {
    tone: "card-cream",
...
// ── Методика по возрасту ───────────────────────────────────────────────────
const ageCards = [
  {
    tone: "card-violet-soft",
    badge: "7–10 лет · интерес через игру",
    body:
      "В младшей школе ребёнок легко различает новые звуки и интонации. Тоны, слова и первые иероглифы вводим через короткие игры, картинки и разговорные ситуации.",
  },
  {
    tone: "card-cream-soft",
    badge: "11–14 лет · интересы и система",
    body:
      "Сохраняем живой контекст — музыку, игры, дунхуа и китайский интернет — и постепенно добавляем учебную систему, домашние задания и цели по HSK.",
  },
  {
    tone: "card-lime-soft",
    badge: "15–17 лет · экзамены и поступление",
    body:
      "Для старшеклассников выстраиваем маршрут к HSK, ОГЭ, ЕГЭ или поступлению. Ранний старт даёт время набрать базу без перегрузки перед экзаменами.",
  },
  {
    tone: "card-sky-soft",
    badge: "Один на один или со сверстниками",
    body:
      "Подбираем индивидуальный темп или мини-группу близких по возрасту детей. Материалы, нагрузка и темы меняются вместе с ребёнком.",
  },
];

// ── Блок №3: дорожная карта класс → уровень → экзамен ─────────────────────
const roadmap = [
  { stage: "1–2 класс · мягкий старт", goal: "Тоны на слух, пиньинь и первые фразы через игры", hsk: "Основа перед HSK 1" },
  { stage: "3–4 класс", goal: "Базовые диалоги, первые иероглифы, 100–150 слов", hsk: "Движение к HSK 1" },
  { stage: "5–8 класс", goal: "Бытовые диалоги, чтение коротких текстов, 150–300 слов", hsk: "HSK 1–2" },
  { stage: "9–11 класс · экзамены", goal: "Аудирование, письменная речь и экзаменационные сценарии", hsk: "Ориентировочно HSK 2–4" },
];
...
// ── Форматы и цена ────────────────────────────────────────────────────────
const priceCards = [
  { tone: "card-cream-soft", title: "Индивидуально для школьника", price: "от 17 990 ₽", note: "1 месяц · 8 занятий по 60 минут", body: "Один на один с преподавателем. После модуля можно продолжить обучение, отдельно оплатив следующий; автоматического списания нет." },
...
export default function ChineseForKidsPage() {
  return (
    <CourseLanding
      ...
      pageHero={{
        variant: "violet",
        eyebrow: "Дети от 7 лет и подростки",
        title: "Курс китайского языка для детей онлайн",
        description:
          "Индивидуально или в мини-группе для детей от 7 лет и подростков. Программа подстраивается под возраст: игровой старт для младших, разговорная практика и HSK 1–2 для старших.",
      }}
      sections={
        <>
          {/* === ЧТО ПОЛУЧАЕТ ШКОЛЬНИК ====================================== */}
          <section className="page-shell-wide section-space">
            <div className="max-w-3xl">
              ...
              <h2 className={`mt-4 ${h2Class}`}>Курс китайского для детей от 7 лет</h2>
              <p className={leadClass}>
                Это полноценная лицензированная программа, которую преподаватель
                адаптирует под возраст ребёнка. В 7–10 лет больше игры и коротких
                заданий; у подростков — разговорная практика и маршрут к HSK 1–2.
              </p>
            </div>
          ...
          <section className="page-shell-wide section-space">
            <div className="max-w-3xl">
              <span className="tag-pill">Методика по возрасту</span>
              <h2 className={`mt-4 ${h2Class}`}>Как меняются занятия с возрастом ребёнка</h2>
              <p className={leadClass}>
                Семилетнему ребёнку, ученику средней школы и старшекласснику нужны
                разные темы, темп и нагрузка. Поэтому программу и преподавателя
                подбираем по возрасту, характеру и цели.
              </p>
            </div>
          ...
          <section className="page-shell-wide section-space">
            <div className="card-block card-block-lg card-cream-soft">
              <h2 className={h2Class}>Зачем ребёнку китайский и с чего начать</h2>
              <div className="mt-6 grid gap-3 text-base leading-[1.65] text-[var(--muted-strong)]">
                <p>
                  Китайский для ребёнка — это не только второй иностранный. Младшим
                  школьникам он развивает слух, память и интерес к другой культуре;
                  подросткам помогает готовиться к HSK, ОГЭ, ЕГЭ и программам с
                  Китаем. Темы подбираем по возрасту — от игр и историй до музыки,
                  технологий и дунхуа.
                </p>
                <p>
                  Начинать можно с 7 лет. Для младших школьников урок строится короткими
                  сменяющимися блоками: звук, игра, картинка, простая фраза и
                  небольшая практика. По мере взросления добавляем иероглифику,
                  системную грамматику, домашние задания и экзаменационные цели.
                </p>
                <p>
      ...
      faqs={[
        {
          ...
          answer:
            "С 7 лет. В младшей школе занятия строятся через игру, наглядные материалы и короткие задания; подросткам добавляем системную грамматику, иероглифику и подготовку к HSK или школьным экзаменам.",
        },
        {
          question: "Сколько стоит детский курс китайского?",
          answer:
            "Индивидуальный модуль для ребёнка от 7 лет — от 17 990 ₽ за один месяц: 8 занятий по 60 минут с русскоязычным преподавателем. Это базовая цена; при преподавателе-носителе или сжатых сроках стоимость выше. После него можно продолжить обучение, отдельно оплатив следующий модуль. Это не подписка и не рассрочка; автоматического списания и обязательной покупки следующих модулей нет. Мини-группа до 5 человек — 15 990 ₽ за 8 занятий.",
        },
        {
        ...
        {
          question: "Нужен ли ребёнку преподаватель-носитель?",
          answer:
      ...
      schemaCourse={{
        ...
        title: "Китайский язык для детей от 7 лет онлайн",
        ...
        priceValue: "15990",
        description:
          "Лицензированный курс китайского языка для детей от 7 лет и подростков. Программа HSK 1–2, разговорный уровень примерно за полгода, подготовка к ОГЭ/ЕГЭ.",
        audience: "Дети от 7 лет и подростки",
        ...
        prerequisites: "Возраст от 7 лет. Предварительная подготовка не требуется.",
